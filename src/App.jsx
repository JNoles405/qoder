...
const store = {
  async get(key) {
    if (typeof window.storage?.get === "function") return window.storage.get(key);
    const v = localStorage.getItem(key); return v ? { value: v } : null;
  },
  async set(key, value) {
    if (typeof window.storage?.set === "function") return window.storage.set(key, value);
...
const TAB_ORDER_KEY  = "qoder-tab-order-v4";
const PROJ_ORDER_KEY = "qoder-proj-order";
...
const TECH_TAGS   = ["React","React Native","Capacitor","Electron","Node.js","Supabase","Firebase","TypeScript","JavaScript","Python","Swift","Kotlin","Flutter","Vue","Next.js","Expo","HTML/CSS","SQLite","PostgreSQL","MongoDB","Tailwind","Vite","Express"];
...
const TIME_PERIODS = [
  { key:"24h", label:"24h",  ms: 1*24*60*60*1000    },
...
function saveTabOrder(o) { localStorage.setItem(TAB_ORDER_KEY, JSON.stringify(o)); }

// ── Folder SVG ─────────────────────────────────────────────────────────────────
function FolderIcon({ size = 13 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 20 16" fill="none">
      <path d="M1 2.5C1 1.67 1.67 1 2.5 1H7.5L9.5 3.5H17.5C18.33 3.5 19 4.17 19 5V13.5C19 14.33 18.33 15 17.5 15H2.5C1.67 15 1 14.33 1 13.5V2.5Z"
        fill="#00d3fe" fillOpacity="0.18" stroke="#00d3fe" strokeWidth="1.4" strokeLinejoin="round"/>
    </svg>
  );
}
...
async function loadProjects(url, key, token, userId) {
  const rows = await sb.get(url,key,token,"projects",`?user_id=eq.${userId}&order=created_at.desc`);
  ...
  return rows.map(p => ({
    ...
    techStack:p.tech_stack||[], localFolder:p.local_folder||null, createdAt:p.created_at,
...
export default function QoderApp() {
      ...
      try {
        ...
        if (saved.session?.refresh_token) {
          ...
          if (res.access_token) {
            ...
            setSession(sess); await persistCfg({url:saved.url,key:saved.key},sess);
            const pjs = await loadProjects(saved.url,saved.key,res.access_token,res.user.id);
            setProjects(pjs); initProjOrder(pjs); setScreen("app"); return;
          }
        }
        setScreen("auth");
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        setProjects(pjs);
        setProjOrder(prev=>{const n=pjs.map(p=>p.id);const k=prev.filter(id=>n.includes(id));const a=n.filter(id=>!k.includes(id));return[...k,...a];});
      } catch {}
    };
  ...
  const initProjOrder = (pjs) => {
    const saved = (()=>{try{const r=localStorage.getItem(PROJ_ORDER_KEY);return r?JSON.parse(r):[];}catch{return[];}})();
    const ids=pjs.map(p=>p.id);
    const valid=saved.filter(id=>ids.includes(id));
    const added=ids.filter(id=>!valid.includes(id));
    setProjOrder([...valid,...added]);
  };
  const saveProjOrder = (order) => { localStorage.setItem(PROJ_ORDER_KEY,JSON.stringify(order)); setProjOrder(order); };
  const orderedProjects = projOrder.map(id=>projects.find(p=>p.id===id)).filter(Boolean);
  ...
  const handleAuth  = async (email,pw,isSignUp) => {
    ...
    try {
      ...
      if (res.access_token) {
        ...
        setProjects(pjs); initProjOrder(pjs); setScreen("app");
  ...
  const addProject = async (p) => {
    try {
      const row=await sb.post(cfg.url,cfg.key,T(),"projects",{user_id:session.user.id,name:p.name,description:p.description||null,status:p.status||"planning",tech_stack:p.techStack||[],local_folder:p.localFolder||null});
      const proj={id:row.id,name:row.name,description:row.description,status:row.status,techStack:row.tech_stack||[],localFolder:row.local_folder||null,createdAt:row.created_at,versions:[],milestones:[],notes:[],todos:[],assets:[],issues:[],ideas:[],concepts:[]};
      setProjects(ps=>[proj,...ps]); setProjOrder(prev=>[row.id,...prev]); showToast("Project created");
    } catch(e){showToast(e.message,"err");}
  };
  const updateProject = async (id,p) => {
    try { await sb.patch(cfg.url,cfg.key,T(),"projects",id,{name:p.name,description:p.description||null,status:p.status,tech_stack:p.techStack||[],local_folder:p.localFolder||null}); mutate(id,x=>({...x,...p})); showToast("Project updated"); }
    catch(e){showToast(e.message,"err");}
  };
  const deleteProject = async (id) => {
    try { await sb.del(cfg.url,cfg.key,T(),"projects",id); setProjects(ps=>ps.filter(p=>p.id!==id)); setProjOrder(prev=>prev.filter(i=>i!==id)); setView("dashboard"); setSelProj(null); showToast("Project deleted"); }
    catch(e){showToast(e.message,"err");}
  };
  ...
  const addVersion = async (pid,v) => {
    try {
      ...
      mutate(pid,p=>({...p,versions:[{id:row.id,version:row.version,releaseNotes:row.release_notes,date:row.date,fileLinks:row.file_links||[]},...p.versions]})); showToast("Version logged");
    } catch(e){showToast(e.message,"err");}
  };
  ...
  // ── Milestones ────────────────────────────────────────────────────────────────
  const addMilestone = async (pid,m) => {
    try {
      const row=await sb.post(cfg.url,cfg.key,T(),"milestones",{project_id:pid,title:m.title,description:m.description||null,date:m.date||null,completed:false});
      mutate(pid,p=>({...p,milestones:[...p.milestones,{id:row.id,title:row.title,description:row.description,date:row.date,completed:false,completedAt:null,createdAt:row.created_at}]})); showToast("Milestone added");
    } catch(e){showToast(e.message,"err");}
  };
  const toggleMilestone = async (pid,mid) => {
    const ms=projects.find(p=>p.id===pid)?.milestones.find(m=>m.id===mid); if(!ms)return;
    const completed=!ms.completed; const completedAt=completed?new Date().toISOString():null;
    try { await sb.patch(cfg.url,cfg.key,T(),"milestones",mid,{completed,completed_at:completedAt}); mutate(pid,p=>({...p,milestones:p.milestones.map(m=>m.id===mid?{...m,completed,completedAt}:m)})); }
    catch(e){showToast(e.message,"err");}
  };
  ...
  const addNote = async (pid,content) => {
    ...
    try { const row=await sb.post(cfg.url,cfg.key,T(),"notes",{project_id:pid,content,position}); mutate(pid,p=>({...p,notes:[...p.notes,{id:row.id,content:row.content,position:row.position,createdAt:row.created_at}]})); showToast("Note added"); }
    catch(e){showToast(e.message,"err");}
  };
  ...
  // ── Todos ─────────────────────────────────────────────────────────────────────
  const addTodo = async (pid,text) => {
    const position=projects.find(p=>p.id===pid)?.todos?.length||0;
    try { const row=await sb.post(cfg.url,cfg.key,T(),"todos",{project_id:pid,text,completed:false,position}); mutate(pid,p=>({...p,todos:[...p.todos,{id:row.id,text:row.text,completed:false,completedAt:null,position:row.position,createdAt:row.created_at}]})); }
    catch(e){showToast(e.message,"err");}
  };
  const toggleTodo = async (pid,tid) => {
    const todo=projects.find(p=>p.id===pid)?.todos.find(t=>t.id===tid); if(!todo)return;
    const completed=!todo.completed; const completedAt=completed?new Date().toISOString():null;
    try { await sb.patch(cfg.url,cfg.key,T(),"todos",tid,{completed,completed_at:completedAt}); mutate(pid,p=>({...p,todos:p.todos.map(t=>t.id===tid?{...t,completed,completedAt}:t)})); }
    catch(e){showToast(e.message,"err");}
  };
  ...
  // ── Issues ────────────────────────────────────────────────────────────────────
  const addIssue = async (pid,iss) => {
    try { const row=await sb.post(cfg.url,cfg.key,T(),"issues",{project_id:pid,title:iss.title,description:iss.description||null,status:"open"}); mutate(pid,p=>({...p,issues:[{id:row.id,title:row.title,description:row.description,status:"open",fixDescription:null,fixedAt:null,createdAt:row.created_at},...p.issues]})); showToast("Issue logged"); }
    catch(e){showToast(e.message,"err");}
  };
  const fixIssue = async (pid,iid,fixDescription) => {
    ...
    try {
      ...
      mutate(pid,p=>({
        ...p,
        issues:p.issues.map(i=>i.id===iid?{...i,status:"fixed",fixDescription,fixedAt}:i),
        notes:[...p.notes,{id:noteRow.id,content:noteRow.content,position:noteRow.position,createdAt:noteRow.created_at}],
      }));
      showToast("Issue marked fixed — note created");
    } catch(e){showToast(e.message,"err");}
  };
  ...
  // ── Ideas ─────────────────────────────────────────────────────────────────────
  const addIdea = async (pid,content) => {
    const position=projects.find(p=>p.id===pid)?.ideas?.length||0;
    try { const row=await sb.post(cfg.url,cfg.key,T(),"ideas",{project_id:pid,content,pinned:false,position}); mutate(pid,p=>({...p,ideas:[...p.ideas,{id:row.id,content:row.content,pinned:false,position:row.position,createdAt:row.created_at}]})); showToast("Idea saved"); }
    catch(e){showToast(e.message,"err");}
  };
  ...
  // ── Nav ───────────────────────────────────────────────────────────────────────
  const openProject = (proj) => { const live=projects.find(p=>p.id===proj.id)||proj; setSelProj(live); setProjTab(tabOrder[0]?.key||"overview"); setView("project"); if(isMobile)setSidebarOpen(false); };
    ...
    <div style={s.root}>
      ...
      <aside style={{...s.sidebar,transform:isMobile?(sidebarOpen?"translateX(0)":"translateX(-100%)"):"translateX(0)",transition:"transform .25s ease",position:isMobile?"fixed":"sticky",zIndex:isMobile?200:1}}>
        <div style={s.logo}>
          ...
          <span style={s.logoBeta}>{APP_VERSION}</span>
        ...
        <nav style={s.nav}>
          ...
          {orderedProjects.length>0&&<>
            ...
            <DraggableSidebarList items={orderedProjects} onReorder={saveProjOrder}>
              {p=><NavBtn active={selProj?.id===p.id&&view==="project"} onClick={()=>openProject(p)} icon={<span style={{color:STATUS_CONFIG[p.status]?.color,fontSize:8}}>●</span>} label={p.name} folder={p.localFolder} small/>}
        ...
        {isMobile && (
          <div style={s.mobileHeader}>
            ...
            <div style={{display:"flex",alignItems:"baseline",gap:2}}>
              <span style={{fontFamily:"'Syne'",fontSize:18,fontWeight:800,color:"#00D4FF"}}>Q</span>
              <span style={{fontFamily:"'Syne'",fontSize:15,fontWeight:700,color:"#E8EAF6"}}>oder</span>
            </div>
            <button className="q-btn-primary" style={{padding:"6px 12px",fontSize:12}} onClick={()=>{openModal("add-project",{status:"planning",techStack:[]});setSidebarOpen(false);}}>+</button>
          </div>
        )}
      ...
      {modal && <ModalWrap onClose={closeModal}>
        ...
        {modal==="add-idea"     && <IdeaForm      data={form} setData={setForm} onSubmit={d=>{addIdea(selProj.id,d.content);closeModal();}}                           onCancel={closeModal}/>}
        {modal==="add-concept"  && <ConceptForm   data={form} setData={setForm} onSubmit={d=>{addConcept(selProj.id,d);closeModal();}}                                onCancel={closeModal}/>}
        {modal==="settings"     && <SettingsModal tabOrder={tabOrder} onSave={order=>{saveTabOrder(order);setTabOrder(order);closeModal();}}                           onCancel={closeModal}/>}
      </ModalWrap>}
    </div>
  );
}
    ...
    <div style={s.authWrap}><style>{css}</style>
      <div style={s.authBox}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontFamily:"'Syne'",fontSize:48,fontWeight:800,color:"#00D4FF",lineHeight:1,marginBottom:4}}>Q</div>
          <div style={{fontFamily:"'Syne'",fontSize:20,fontWeight:800,color:"#E8EAF6"}}>Connect Supabase</div>
        </div>
...
function AuthScreen({ onAuth, busy, onReset }) {
  ...
  return (
    <div style={s.authWrap}><style>{css}</style>
      <div style={s.authBox}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{display:"flex",justifyContent:"center",alignItems:"baseline",gap:2,marginBottom:6}}><span style={{fontFamily:"'Syne'",fontSize:40,fontWeight:800,color:"#00D4FF"}}>Q</span><span style={{fontFamily:"'Syne'",fontSize:30,fontWeight:700,color:"#E8EAF6",letterSpacing:"-.5px"}}>oder</span></div>
          <p style={{color:"#6B7290",fontSize:13}}>{isSignUp?"Create your account":"Sign in to your workspace"}</p>
        </div>
        ...
        <Field label="Password">
          <div style={{position:"relative"}}><input className="q-input" type={showPw?"text":"password"} value={pw} onChange={e=>setPw(e.target.value)} placeholder="••••••••" style={{paddingRight:44}} onKeyDown={e=>e.key==="Enter"&&onAuth(email,pw,isSignUp)}/>
          <button onClick={()=>setShowPw(v=>!v)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",color:"#6B7290",fontSize:12,background:"none",border:"none",cursor:"pointer"}}>{showPw?"hide":"show"}</button></div>
        </Field>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:16,fontSize:13}}>
          <button onClick={()=>setIsSignUp(v=>!v)} style={{color:"#00D4FF",background:"none",border:"none",cursor:"pointer"}}>{isSignUp?"Already have an account?":"Create an account"}</button>
          <button onClick={onReset} style={{color:"#4B5268",background:"none",border:"none",cursor:"pointer",fontSize:12}}>Change project</button>
        </div>
      </div>
    </div>
  );
...
function NavBtn({ active, onClick, icon, label, small, folder }) {
  ...
  return (
    <div className={`q-nav${active?" q-nav-active":""}`} onClick={onClick} style={{fontSize:small?12:13,paddingLeft:small?24:14,display:"flex",alignItems:"center"}}>
      <span style={{fontSize:small?8:13,marginRight:6,display:"flex",alignItems:"center",flexShrink:0}}>{icon}</span>
...
function DraggableSidebarList({ items, onReorder, children }) {
  ...
  const onDrop=()=>{onReorder(list.map(p=>p.id));setDragIdx(null);};
  ...
  return (
    <div style={{...s.page,padding:isMobile?"16px 14px":"36px 40px"}}>
      <div style={{...s.pageHead,marginBottom:isMobile?16:26}}>
        <div><h1 style={{...s.pageTitle,fontSize:isMobile?20:27}}>Dashboard</h1><p style={s.pageSub}>Cloud-synced across all your devices</p></div>
      ...
      {projects.length===0?(
        <div style={s.empty}><div style={s.emptyIcon}>⬡</div><p>{allProjects.length===0?"No projects yet.":"No projects match your filters."}</p><button className="q-btn-primary" onClick={onNew}>+ New Project</button></div>
      ):(
...
function ProjectView({ project, tab, setTab, isMobile, tabOrder, onAddVersion, onAddMilestone, onToggleMilestone, onDeleteMilestone, onDeleteVersion, onAddNote, onEditNote, onDeleteNote, onReorderNotes, onAddTodo, onToggleTodo, onDeleteTodo, onReorderTodos, onAddAsset, onDeleteAsset, onAddIssue, onFixIssue, onDeleteIssue, onAddIdea, onToggleIdeaPin, onDeleteIdea, onReorderIdeas, onAddConcept, onDeleteConcept, onEdit, onDelete }) {
  ...
  return (
    <div style={{...s.page,padding:isMobile?"14px":"36px 40px"}}>
      <div style={{...s.projHead,marginBottom:isMobile?14:24}}>
      ...
      {tab==="assets"     && <AssetsTab     project={project} onAdd={onAddAsset}       onDelete={onDeleteAsset}/>}
      {tab==="issues"     && <IssuesTab     project={project} onAdd={onAddIssue}       onFix={onFixIssue}           onDelete={onDeleteIssue}/>}
      {tab==="ideas"      && <IdeasTab      project={project} onAdd={onAddIdea}        onPin={onToggleIdeaPin}      onDelete={onDeleteIdea}      onReorder={onReorderIdeas}/>}
      {tab==="concepts"   && <ConceptsTab   project={project} onAdd={onAddConcept}     onDelete={onDeleteConcept}/>}
    </div>
  );
}
...
function OverviewTab({ project, latestVer }) {
  ...
  const msDone=project.milestones?.filter(m=>m.completed).length||0;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        <input className="q-input" style={{flex:1,minWidth:160,maxWidth:280,marginTop:0}} placeholder="Search feed…" value={search} onChange={e=>setSearch(e.target.value)}/>
        <button className="q-chip" style={{fontFamily:"'JetBrains Mono'",fontSize:11}} onClick={()=>setSortDir(d=>d==="desc"?"asc":"desc")}>
          {sortDir==="desc"?"↓ Newest":"↑ Oldest"}
        </button>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {TIME_PERIODS.map(p=>(
            <button key={p.key} className={`q-chip${period===p.key?" q-chip-on":""}`} style={{fontSize:11,padding:"3px 9px"}} onClick={()=>setPeriod(p.key)}>{p.label}</button>
          ))}
        </div>
      </div>
      ...
      {/* Feed */}
      {items.length===0 ? (
        <div style={s.empty}><p>No activity for this period.</p></div>
      ) : (
...
function FeedItem({ item }) {
  const meta=FEED_META[item.type]||FEED_META.note;
  return (
    <div style={{display:"flex",gap:12,padding:"12px 14px",background:"#111627",border:"1px solid #1A2040",borderRadius:10,alignItems:"flex-start"}}>
      ...
      <div style={{...s.mono10,whiteSpace:"nowrap",flexShrink:0,marginTop:4}}>
          ...
          {project.versions.map((v,i)=>(
            <div key={v.id} className="q-ver-card">
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}><span style={s.mono10}>{new Date(v.date).toLocaleDateString()}</span><button className="q-del" onClick={()=>{if(confirm("Remove version?"))onDelete(v.id);}}>✕</button></div>
              </div>
...
function TodoRow({ todo, onToggle, onDelete }) {
  return (
    <div className="q-ms-row" style={{opacity:todo.completed?.5:1}}>
      ...
      <div style={{flex:1}}><span style={{color:"#E8EAF6",fontWeight:500,textDecoration:todo.completed?"line-through":"none",fontSize:14}}>{todo.text}</span>
...
function NotesTab({ project, onAdd, onEdit, onDelete, onReorder }) {
  ...
  return (
    <div>
      <div style={s.tabBar}><span style={s.mono12}>{notes.length} {notes.length===1?"note":"notes"}</span><button className="q-btn-primary" onClick={onAdd}>+ Add Note</button></div>
      {notes.length===0?<div style={s.empty}><p>No notes yet.</p></div>:(
        <DraggableList items={notes} onReorder={onReorder}>
          {note=><div className="q-ver-card" style={{marginBottom:10}}>
            <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
              <span style={{color:"#3A4060",fontSize:18,cursor:"grab",userSelect:"none",flexShrink:0,marginTop:2}}>⠿</span>
              <p style={{color:"#B8BDD4",fontSize:14,lineHeight:1.75,whiteSpace:"pre-wrap",flex:1}}>{note.content}</p>
              <div style={{display:"flex",gap:6,flexShrink:0}}><button className="q-btn-ghost" style={{padding:"4px 10px",fontSize:12}} onClick={()=>onEdit(note)}>Edit</button><button className="q-del" onClick={()=>{if(confirm("Delete?"))onDelete(note.id);}}>✕</button></div>
            </div>
...
// ── Assets Tab ────────────────────────────────────────────────────────────────
function AssetsTab({ project, onAdd, onDelete }) {
  const assets=project.assets||[];
  const grouped=ASSET_TYPES.reduce((acc,type)=>{const items=assets.filter(a=>a.type===type);if(items.length>0)acc[type]=items;return acc;},{});
  return (
    <div>
      <div style={s.tabBar}><span style={s.mono12}>{assets.length} {assets.length===1?"asset":"assets"}</span><button className="q-btn-primary" onClick={onAdd}>+ Add Asset</button></div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          {Object.entries(grouped).map(([type,items])=>(
            <div key={type}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {items.map(asset=><div key={asset.id} className="q-ms-row"><div style={{flex:1,minWidth:0}}><div style={{color:"#E8EAF6",fontWeight:500,fontSize:14}}>{asset.name}</div><a href={asset.url} target="_blank" rel="noreferrer" style={{...s.fileLink,marginTop:5,display:"inline-block",maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>↗ {asset.url}</a></div><button className="q-del" onClick={()=>{if(confirm("Remove?"))onDelete(asset.id);}}>✕</button></div>)}
              </div>
            </div>
          ))}
        </div>
...
function IssuesTab({ project, onAdd, onFix, onDelete }) {
  ...
  return (
    <div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                    <span style={{width:8,height:8,borderRadius:"50%",background:"#FF6B9D",flexShrink:0,display:"inline-block"}}/>
                    <span style={{color:"#E8EAF6",fontWeight:600,fontSize:14}}>{iss.title}</span>
                  </div>
                  {iss.description&&<p style={{color:"#8B8FA8",fontSize:13,lineHeight:1.6,marginLeft:16}}>{iss.description}</p>}
                ...
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  ...
                  <button className="q-del" onClick={()=>{if(confirm("Remove issue?"))onDelete(iss.id);}}>✕</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {fixed.length>0&&(
        <>
          <div style={{fontFamily:"'JetBrains Mono'",fontSize:10,color:"#4B5268",letterSpacing:1.2,textTransform:"uppercase",fontWeight:700,marginBottom:10}}>Fixed ({fixed.length})</div>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {fixed.map(iss=>(
              <div key={iss.id} className="q-ms-row" style={{opacity:.55}}>
                <span style={{width:8,height:8,borderRadius:"50%",background:"#4ADE80",flexShrink:0,marginTop:5}}/>
                <div style={{flex:1}}>
                  <span style={{color:"#E8EAF6",fontSize:13,fontWeight:500,textDecoration:"line-through"}}>{iss.title}</span>
                  {iss.fixDescription&&<p style={{color:"#6B7290",fontSize:12,marginTop:2}}>{iss.fixDescription}</p>}
                  <div style={{...s.mono10,marginTop:3,color:"#2E3558"}}>{iss.fixedAt?new Date(iss.fixedAt).toLocaleDateString():""}</div>
                </div>
                <button className="q-del" onClick={()=>{if(confirm("Remove?"))onDelete(iss.id);}}>✕</button>
              </div>
            ))}
          </div>
        </>
...
// ── Ideas Tab ─────────────────────────────────────────────────────────────────
function IdeasTab({ project, onAdd, onPin, onDelete, onReorder }) {
  const ideas  = project.ideas||[];
  const pinned = ideas.filter(d=>d.pinned);
  const rest   = ideas.filter(d=>!d.pinned);
  return (
    <div>
      ...
      {ideas.length===0&&<div style={s.empty}><p>No ideas yet. This tab stays off Overview.</p></div>}
      {pinned.length>0&&(
        <>
          ...
          {pinned.map(idea=><IdeaRow key={idea.id} idea={idea} onPin={()=>onPin(idea.id)} onDelete={()=>{if(confirm("Remove?"))onDelete(idea.id);}}/>)}
          {rest.length>0&&<div style={{height:1,background:"#1A2040",margin:"16px 0"}}/>}
        </>
      )}
      {rest.length>0&&(
        <DraggableList items={rest} onReorder={r=>onReorder([...pinned,...r])}>
          {idea=><IdeaRow idea={idea} onPin={()=>onPin(idea.id)} onDelete={()=>{if(confirm("Remove?"))onDelete(idea.id);}}/>}
        </DraggableList>
      )}
    </div>
  );
}
function IdeaRow({ idea, onPin, onDelete }) {
  return (
    <div className="q-ver-card" style={{marginBottom:8}}>
      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
        <span style={{color:"#3A4060",fontSize:18,cursor:"grab",userSelect:"none",flexShrink:0,marginTop:2}}>⠿</span>
        <p style={{color:"#B8BDD4",fontSize:14,lineHeight:1.7,whiteSpace:"pre-wrap",flex:1}}>{idea.content}</p>
        <div style={{display:"flex",gap:4,flexShrink:0}}>
          <button onClick={onPin} title={idea.pinned?"Unpin":"Pin"} style={{fontSize:15,padding:"2px 4px",opacity:idea.pinned?1:.4,transition:"opacity .15s",background:"none",border:"none",cursor:"pointer"}}>⭐</button>
          <button className="q-del" onClick={onDelete}>✕</button>
        </div>
      </div>
      <div style={{...s.mono10,marginTop:8,color:"#2E3558"}}>{new Date(idea.createdAt).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"})}</div>
    </div>
  );
}

// ── Concepts Tab ──────────────────────────────────────────────────────────────
function ConceptsTab({ project, onAdd, onDelete }) {
  ...
  return (
    <div>
      <div style={s.tabBar}><span style={s.mono12}>{concepts.length} {concepts.length===1?"concept":"concepts"}</span><button className="q-btn-primary" onClick={onAdd}>+ Add Concept</button></div>
          ...
          {Object.entries(byType).map(([type,items])=>(
            <div key={type}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
                {items.map(c=><ConceptCard key={c.id} concept={c} onDelete={()=>{if(confirm("Remove?"))onDelete(c.id);}}/>)}
              </div>
            </div>
          ))}
        </div>
...
function ConceptCard({ concept, onDelete }) {
  const isColor=concept.type==="color"; const isImage=concept.type==="image";
  const isAudio=concept.type==="audio"; const isCode=concept.type==="code"; const isLink=concept.type==="link";
  return (
    <div style={{background:"#111627",border:"1px solid #1A2040",borderRadius:10,padding:14,position:"relative",transition:"border-color .2s"}} className="q-ver-card">
      ...
      {isImage&&<img src={concept.content} alt={concept.label||"concept"} style={{width:"100%",borderRadius:6,objectFit:"cover",maxHeight:180,display:"block"}} onError={e=>e.target.style.display="none"}/>}
...
function SettingsModal({ tabOrder, onSave, onCancel }) {
  ...
  return (
    <div>
      ...
      <p style={{fontSize:12,color:"#4B5268",marginTop:12}}>Tab order is saved locally on this device.</p>
...
function VersionForm({ data, setData, onSubmit, onCancel }) {
  ...
  return (
    <div>
      <h2 style={s.modalTitle}>Log New Version</h2>
      <Field label="Version Number *"><input className="q-input q-mono" value={data.version||""} onChange={e=>set("version",e.target.value)} placeholder="e.g., v1.2.0"/></Field>
      <Field label="Release Date"><input type="date" className="q-input" value={data.date?.split("T")[0]||today} onChange={e=>set("date",e.target.value)}/></Field>
      <Field label="Release Notes"><textarea className="q-input" style={{height:90,resize:"vertical"}} value={data.releaseNotes||""} onChange={e=>set("releaseNotes",e.target.value)} placeholder="What changed?"/></Field>
      ...
      <FormActions onCancel={onCancel} onSubmit={()=>data.version?.trim()&&onSubmit(data)} submitLabel="Log Version"/>
    </div>
  );
}
function MilestoneForm({ data, setData, onSubmit, onCancel }) {
  ...
  return (
    <div>
      <h2 style={s.modalTitle}>Add Milestone</h2>
      <Field label="Title *"><input className="q-input" value={data.title||""} onChange={e=>set("title",e.target.value)} placeholder="e.g., Submit to Play Store"/></Field>
      <Field label="Target Date"><input type="date" className="q-input" value={data.date||""} onChange={e=>set("date",e.target.value)}/></Field>
      <Field label="Description"><textarea className="q-input" style={{height:72,resize:"vertical"}} value={data.description||""} onChange={e=>set("description",e.target.value)} placeholder="Optional…"/></Field>
      <FormActions onCancel={onCancel} onSubmit={()=>data.title?.trim()&&onSubmit(data)} submitLabel="Add Milestone"/>
    </div>
  );
}
function NoteForm({ data, setData, title, onSubmit, onCancel }) {
  ...
  return (
    <div>
      <h2 style={s.modalTitle}>{title}</h2>
      <Field label="Content *"><textarea className="q-input" style={{height:160,resize:"vertical"}} autoFocus value={data.content||""} onChange={e=>set("content",e.target.value)} placeholder="Write your note…"/></Field>
      <FormActions onCancel={onCancel} onSubmit={()=>data.content?.trim()&&onSubmit(data)} submitLabel="Save Note"/>
    </div>
  );
}
function AssetForm({ data, setData, onSubmit, onCancel }) {
  ...
  return (
    <div>
      ...
      <Field label="Name *"><input className="q-input" value={data.name||""} onChange={e=>set("name",e.target.value)} placeholder="e.g., Play Store Icon"/></Field>
      <Field label="URL *"><input className="q-input" value={data.url||""} onChange={e=>set("url",e.target.value)} placeholder="https://…"/></Field>
      <Field label="Type"><select className="q-input" value={data.type||"Link"} onChange={e=>set("type",e.target.value)}>{ASSET_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></Field>
...
function IssueForm({ data, setData, onSubmit, onCancel }) {
  ...
  return (
    <div>
      ...
      <Field label="Description"><textarea className="q-input" style={{height:100,resize:"vertical"}} value={data.description||""} onChange={e=>set("description",e.target.value)} placeholder="Steps to reproduce, expected vs actual…"/></Field>
  ...
  return (
    <div>
      ...
      <div style={{background:"rgba(74,222,128,.06)",border:"1px solid rgba(74,222,128,.15)",borderRadius:8,padding:"10px 14px",marginBottom:16}}>
        ...
        {data.description&&<p style={{color:"#8B8FA8",fontSize:12,marginTop:4}}>{data.description}</p>}
      </div>
      <Field label="How was it fixed? *"><textarea className="q-input" style={{height:120,resize:"vertical"}} autoFocus value={data.fixDescription||""} onChange={e=>set("fixDescription",e.target.value)} placeholder="Describe the fix…"/></Field>
      <p style={{fontSize:12,color:"#6B7290",marginTop:6}}>A note will be automatically created with this fix details.</p>
      <FormActions onCancel={onCancel} onSubmit={()=>data.fixDescription?.trim()&&onSubmit(data)} submitLabel="Mark Fixed"/>
    </div>
  );
}
function IdeaForm({ data, setData, onSubmit, onCancel }) {
  const set=(k,v)=>setData(d=>({...d,[k]:v}));
  return (
    <div>
      <h2 style={s.modalTitle}>Add Idea</h2>
      <Field label="Idea *"><textarea className="q-input" style={{height:140,resize:"vertical"}} autoFocus value={data.content||""} onChange={e=>set("content",e.target.value)} placeholder="Your idea…"/></Field>
      <p style={{fontSize:12,color:"#6B7290",marginTop:4}}>Ideas don't appear on Overview — only in this tab.</p>
      <FormActions onCancel={onCancel} onSubmit={()=>data.content?.trim()&&onSubmit(data)} submitLabel="Save Idea"/>
    </div>
  );
}
function ConceptForm({ data, setData, onSubmit, onCancel }) {
  const set=(k,v)=>setData(d=>({...d,[k]:v}));
  ...
  const startRec=async()=>{
    try{
      ...
      mr.onstop=()=>{ const blob=new Blob(chunks,{type:"audio/webm"}); const reader=new FileReader(); reader.onload=()=>set("content",reader.result); reader.readAsDataURL(blob); stream.getTracks().forEach(t=>t.stop()); };
  ...
  const renderInput=()=>{
    switch(data.type){
      case "color": return <div style={{display:"flex",gap:10,alignItems:"center",marginTop:8}}><input type="color" value={data.content||"#00D4FF"} onChange={e=>set("content",e.target.value)} style={{width:48,height:40,padding:2,background:"#0D1120",border:"1px solid #1E2540",borderRadius:6,cursor:"pointer"}}/><input className="q-input" style={{flex:1,marginTop:0,fontFamily:"'JetBrains Mono'"}} value={data.content||""} onChange={e=>set("content",e.target.value)} placeholder="#hex, rgb(), or hsl()"/></div>;
      case "image": return <input className="q-input" value={data.content||""} onChange={e=>set("content",e.target.value)} placeholder="https://image-url.com/..."/>;
      case "audio": return <div style={{marginTop:8}}>{recording?<button className="q-btn-danger" style={{width:"100%",padding:10}} onClick={stopRec}>⏹ Stop Recording</button>:<button className="q-btn-ghost" style={{width:"100%",padding:10}} onClick={startRec}>🎙 Start Recording</button>}{data.content&&<audio src={data.content} controls style={{width:"100%",marginTop:10}}/>}</div>;
  ...
  return (
    <div>
      <h2 style={s.modalTitle}>Add Concept</h2>
      <Field label="Type"><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>{CONCEPT_TYPES.map(t=><button key={t} className={`q-chip${data.type===t?" q-chip-on":""}`} onClick={()=>set("type",t)}>{CONCEPT_ICONS[t]} {t}</button>)}</div></Field>
      <Field label="Label (optional)"><input className="q-input" value={data.label||""} onChange={e=>set("label",e.target.value)} placeholder="Name this concept…"/></Field>
      <Field label={data.type==="color"?"Color":data.type==="image"?"Image URL":data.type==="audio"?"Voice Recording":data.type==="code"?"Code Snippet":data.type==="link"?"Link":"Text"}>{renderInput()}</Field>
      <p style={{fontSize:12,color:"#6B7290",marginTop:4}}>Concepts are a mood board — they don't appear on Overview.</p>
      <FormActions onCancel={onCancel} onSubmit={()=>data.content?.trim()&&onSubmit(data)} submitLabel="Add Concept"/>
    </div>
  );
}

function Field({label,children}){return<div style={{marginBottom:14}}><label style={s.fieldLbl}>{label}</label>{children}</div>;}
function FormActions({onCancel,onSubmit,submitLabel}){return<div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:22}}><button className="q-btn-ghost" onClick={onCancel}>Cancel</button><button className="q-btn-primary" onClick={onSubmit}>{submitLabel}</button></div>;}
function ModalWrap({children,onClose}){return<div style={s.overlayBackdrop} onClick={onClose}><div style={s.modal} onClick={e=>e.stopPropagation()}>{children}</div></div>;}
...
const css=`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .q-nav{display:flex;align-items:center;padding:9px 14px;color:#6B7290;font-family:'Syne',sans-serif;font-weight:500;width:calc(100% - 16px);margin:1px 8px;border-radius:7px;cursor:pointer;transition:all .15s;user-select:none;}
  .q-nav:hover{background:rgba(0,212,255,.06);color:#C0C6E0;}.q-nav-active{background:rgba(0,212,255,.09)!important;color:#00D4FF!important;}
...
const s={
  ...
  sidebar:{width:220,minWidth:220,background:"#0C1020",borderRight:"1px solid #151C32",display:"flex",flexDirection:"column",top:0,height:"100vh",overflowY:"auto"},
  ...
  overlayBackdrop:{position:"fixed",inset:0,background:"rgba(4,6,14,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000},
